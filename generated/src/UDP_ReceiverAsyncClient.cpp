...
uint64_t UDP_ReceiverAsyncClient::vnx_get_module_info(const std::function<void(std::shared_ptr<const ::vnx::ModuleInfo>)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
uint64_t UDP_ReceiverAsyncClient::vnx_stop(const std::function<void()>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
uint64_t UDP_ReceiverAsyncClient::vnx_self_test(const std::function<void(const vnx::bool_t&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
int32_t UDP_ReceiverAsyncClient::vnx_purge_request(uint64_t _request_id, const vnx::exception& _ex) {
...
int32_t UDP_ReceiverAsyncClient::vnx_callback_switch(uint64_t _request_id, std::shared_ptr<const vnx::Value> _value) {
	std::unique_lock<std::mutex> _lock(vnx_mutex);
	const auto _iter = vnx_pending.find(_request_id);
	if(_iter == vnx_pending.end()) {
		throw std::runtime_error("UDP_ReceiverAsyncClient: received unknown return");
	}
	const auto _index = _iter->second;
	vnx_pending.erase(_iter);
	switch(_index) {
		case 0: {
			const auto _iter = vnx_queue_vnx_get_config_object.find(_request_id);
			if(_iter == vnx_queue_vnx_get_config_object.end()) {
				throw std::runtime_error("UDP_ReceiverAsyncClient: callback not found");
			}
			...
			if(_callback) {
				if(auto _result = std::dynamic_pointer_cast<const ::vnx::ModuleInterface_vnx_get_config_object_return>(_value)) {
					...
					throw std::logic_error("UDP_ReceiverAsyncClient: invalid return value");
				}
			}
			break;
		}
		...
		default:
			if(_index >= 0) {
				throw std::logic_error("UDP_ReceiverAsyncClient: invalid callback index");
			}
	}
	return _index;
}
