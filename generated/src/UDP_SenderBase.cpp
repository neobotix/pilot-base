...
const vnx::Hash64 UDP_SenderBase::VNX_CODE_HASH(0xe5ee57c510ffa8c9ull);
	...
	:	Module::Module(_vnx_name)
{
	...
	vnx::read_config(vnx_name + ".max_receive_packet_size", max_receive_packet_size);
...
void UDP_SenderBase::accept(vnx::Visitor& _visitor) const {
	...
	_visitor.type_field(_type_code->fields[4], 4); vnx::accept(_visitor, max_receive_packet_size);
	_visitor.type_field(_type_code->fields[5], 5); vnx::accept(_visitor, read_timeout_ms);
	_visitor.type_field(_type_code->fields[6], 6); vnx::accept(_visitor, error_interval_ms);
	_visitor.type_field(_type_code->fields[7], 7); vnx::accept(_visitor, stats_interval_ms);
...
void UDP_SenderBase::write(std::ostream& _out) const {
	...
	_out << ", \"max_receive_packet_size\": "; vnx::write(_out, max_receive_packet_size);
...
vnx::Object UDP_SenderBase::to_object() const {
	...
	_object["port"] = port;
	_object["max_receive_packet_size"] = max_receive_packet_size;
...
void UDP_SenderBase::from_object(const vnx::Object& _object) {
	for(const auto& _entry : _object.field) {
		...
		} else if(_entry.first == "max_receive_packet_size") {
			_entry.second.to(max_receive_packet_size);
...
vnx::Variant UDP_SenderBase::get_field(const std::string& _name) const {
	...
	if(_name == "max_receive_packet_size") {
		return vnx::Variant(max_receive_packet_size);
	}
...
void UDP_SenderBase::set_field(const std::string& _name, const vnx::Variant& _value) {
	if(_name == "input") {
		...
		_value.to(port);
	} else if(_name == "max_receive_packet_size") {
		_value.to(max_receive_packet_size);
...
std::shared_ptr<vnx::TypeCode> UDP_SenderBase::static_create_type_code() {
	...
	type_code->code_hash = vnx::Hash64(0xe5ee57c510ffa8c9ull);
	...
	}
	{
		...
		field.data_size = 4;
		field.name = "max_receive_packet_size";
...
void read(TypeInput& in, ::pilot::base::UDP_SenderBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	if(type_code->is_matched) {
		...
		if(const auto* const _field = type_code->field_map[4]) {
			vnx::read_value(_buf + _field->offset, value.max_receive_packet_size, _field->code.data());
		}
...
void write(TypeOutput& out, const ::pilot::base::UDP_SenderBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	vnx::write_value(_buf + 4, value.max_receive_packet_size);
