...
/**
 * \file j1587lib.h
 * \brief Definitions for the J1587lib API.
 * \details
 * \defgroup grp_j1587 J1587
 * \brief Definitions for the J1587lib API.
 * \defgroup j1587_status_codes Status Codes
 * \brief  Return codes from the j1587 functions
 * \ingroup grp_j1587
 * \defgroup j1587_general General
 * \ingroup grp_j1587
 */

/**
 * \page page_j1587 J1587 API (J1587lib)
 *
  * \section section_user_guide_j1587_1 Description
 *
 * The J1587 API is used by an application in conjuction with the Kvaser Linx
 * J1587 for automobile communication.
 *
 * \section section_user_guide_j1587_where_to_go_from_here Where to go from here
 * For more details, see the module \ref grp_j1587.
 *
 */
 ...
 * is used by \ref canOpenChannel() to access the LAPcan channel.
 *
 * \param[in] channel  The number of the channel. This is the same as used 
 *                     by \ref canOpenChannel() to access the LAPcan channel. 
 ...
 *
 * \return \ref j1587OK (zero) if success
 * \return \ref j1587ERR_xxx (negative) if failure
 *
 * \sa \ref j1587OpenChannel()
 */
 ...
 * integer - e.g. 0xFFFFFFF2 for \ref j1587ERR_INVHANDLE. Watch out. 
 *
 * \param[in] h        A handle to an open J1587 channel.
 ...
 * 
 * \return \ref j1587OK (zero) if success
 * \return \ref j1587ERR_NOMSG if nothing is received
 * \return \ref j1587ERR_xxx (negative) if failure
 */
 ...
 * \param[in] timeout     Times out after \a timeout milliseconds
 *
 * \return \ref j1587OK (zero) if all writes are done
 * \return \ref j1587ERR_xxx (negative) if failure
 */
